...
EOF;
  ...
  protected function execute($arguments = array(), $options = array())
  {
    ...
    while($it->valid())
    {
      ...
      if(strpos($path,'model/om')||strpos($path,'model/map')
        ||strpos($path,'form/base')||strpos($path,'filter/base')
        ||strpos($path,'doctrine/base')||strpos($path,'Plugin/base'))
      {
  ...
  private function writeIgnore($currentIgnore,$ignoreFiles,$addIgnore, $forceAddIgnoreFileCommand)
  {
	...
	$this->log($currentIgnore.' writing');
